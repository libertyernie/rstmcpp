...
namespace RSTMCPP {
	namespace AudioConverter {
		void CalcCoefs(int16_t* source, int samples, int16_t* dest, ProgressTracker* progress) {
			...
			double* bufferArray[8];
			for (int z = 0; z < 8; z++)
				bufferArray[z] = (double*)malloc(3 * 8);

			double buffer2[3];
			short* sampleBuffer = (short*)malloc(0x7000);
			short chunkBuffer[28];
			//double** chunkBuffer = (double**)chunkBufferData;
			for (int z = 0; z < 28; z++)
				chunkBuffer[z] = 0;

			double sChunkBuffer[3];
			double omgBuffer[3];

			double* pChannels[3];
			for (int z = 0; z <= 2; z++)
				pChannels[z] = (double*)malloc(3 * 8);

			int anotherBuffer[3];

			double** multiBuffer = (double**)malloc(numBlocks * 4 * 2);
			int unused = 0;
			int multiIndex = 0;
			...
			for (int x = samples; x > 0;)
			{
				...
				for (int i = 0; i < blockSamples;)
				{
					...
					if (std::abs(sChunkBuffer[0]) > 10.0)
					{
						...
						if (!Something3(pChannels, anotherBuffer, &temp))
						{
							...
							{
								multiBuffer[multiIndex] = (double*)malloc(3 * 8);
		...
		void EncodeChunk(int16_t* source, int samples, uint8_t* dest, int16_t* coefs) {
			...
			int64_t bestDistance = 0x7FFFFFFFFFFFFFFFLL, distAccum;
			int32_t bestIndex = 0;
			int32_t bestScale = 0;

			int32_t distance, index, scale;
			...
			for (int i = 0; i < 8; i++, p1 += 16, p2 += 14, coefs += 2)
			{
				...
				*t1++ = source[1];
				...
				for (int y = 0; y < samples; y++)
				{
					...
					int16_t sample1 = *sPtr++;
					int16_t sample2 = *sPtr++;
					*t1++ = v1 = ((sample1 * coefs[1]) + (sample2 * coefs[0])) >> 11;
					//Subtract from current sample
					v2 = *sPtr-- - v1;
					//Clamp
					v3 = (v2 >= 32767) ? 32767 : (v2 <= -32768) ? -32768 : v2;
					//Compare distance
					if (std::abs(v3) > std::abs(distance))
						distance = v3;
				}
				...
				do
				{
					...
					for (int y = 0; y < samples; y++)
					{
                        ...
                        int32_t t1_1 = *t1++;
						int32_t t1_2 = *t1++;
						v1 = ((t1_1 * coefs[1]) + (t1_2 * coefs[0]));
						//Evaluate from real sample
						v2 = ((*sPtr << 11) - v1) / 2048;
						//Round to nearest sample
						v3 = (v2 > 0) ? (int32_t)((double)v2 / (1 << scale) + 0.4999999f) : (int32_t)((double)v2 / (1 << scale) - 0.4999999f);
			...
			//Write ps
			*dest++ = (uint8_t)((bestIndex << 4) | (bestScale & 0xF));
			...
			for (int y = 0; y++ < 7;) {
				int32_t p2_1 = *p2++;
				int32_t p2_2 = *p2++;
				*dest++ = (uint8_t)((p2_1 << 4) | (p2_2 & 0xF));
			}
		}

		void Something10(double** bufferArray, int mask, double** multiBuffer, int multiIndex, double val)
		{
			double** bufferList = (double**)malloc(mask * sizeof(double*));

			int* buffer1 = (int*)malloc(mask * sizeof(int));
			double buffer2[3];
			...
			for (int i = 0; i < mask; i++)
				bufferList[i] = (double*)malloc(8 * 3);
